...
public class MySqlMigrationTests : MySqlTestBase
{
    ...
    public async Task Should_Migrate_JSON_Data_Correctly()
    {
        ...
        organization.GetType().GetProperty("LocationIds")?.SetValue(organization, locationIds);
        await DbContext.SaveChangesAsync();
        ...
        var orgsWithLocations = await DbContext.Organizations
            .Where(o => o.LocationIds.Count > 0)
            .ToListAsync();
        
        Assert.AreEqual(1, orgsWithLocations.Count);
    }
    ...
    public async Task Should_Migrate_Concurrency_Tokens()
    {
        ...
        org1!.UpdateDetails("Updated by migration 1", "Updated description 1", "test1@example.com", "555-0001", "https://test1.com", "migration-user");
        await DbContext.SaveChangesAsync();
        ...
        {
            org2!.UpdateDetails("Updated by migration 2", "Updated description 2", "test2@example.com", "555-0002", "https://test2.com", "migration-user");
            await DbContext.SaveChangesAsync();
        });
    }
    ...
    public async Task Should_Migrate_Data_With_Empty_Strings()
    {
        ...
        Assert.AreEqual("", savedOrg.ContactEmail);
        Assert.AreEqual("", savedOrg.ContactPhone);
